...
//  NSData *commands = [builder.commands            copy];
    ...
    if (_scaleStatus == ScaleStatusConnect) {
        dispatch_async(GlobalQueueManager.sharedManager.serialQueue, ^{
            [Communication sendCommandsDoNotCheckCondition:commands port:_port completionHandler:^(BOOL result, NSString *title, NSString *message) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    if (result == NO) {
...
//          BOOL portValid = NO;
            ...
            if ([_lock tryLock]) {
                ...
                if (portValid == YES) {
                    ...
                    dispatch_async(GlobalQueueManager.sharedManager.serialQueue, ^{
//                      [Communication      parseDoNotCheckCondition:weightParser port:_port completionHandler:^(BOOL result, NSString *title, NSString *message) {
                        [ScaleCommunication parseDoNotCheckCondition:weightParser port:_port completionHandler:^(BOOL result, NSString *title, NSString *message) {
                            if (result == YES) {
                                dispatch_async(dispatch_get_main_queue(), ^{
...
//                                      case StarIoExtDisplayedWeightStatusZero :
                                            _commentLabel.text = weightParser.weight;
...
//                                          _commentLabel.textColor = [UIColor greenColor];
                                ...
                                [Communication parseDoNotCheckCondition:parser port:_port completionHandler:^(BOOL result, NSString *title, NSString *message) {
                                    dispatch_async(dispatch_get_main_queue(), ^{
                                        if (result == YES) {
                                            if (parser.connect == YES) {     // Because the scale doesn't sometimes react.
                                            }
                                            else {
